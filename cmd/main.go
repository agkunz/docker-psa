...
func main() {
	...
	debugFile, _ := os.OpenFile("/tmp/docker-psa-debug.log", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if debugFile != nil {
...
func filterContainers(containers []types.Container, pattern string) []types.Container {
...
func printContainersMultiline(containers []types.Container, verboseLevel int) {
	if len(containers) == 0 {
...
func formatPortsMultiLine(ports []types.Port) string {
	if len(ports) == 0 {
		return ""
	}
...
func formatPortsVerbose(ports []types.Port) string {
	if len(ports) == 0 {
		return ""
	}
